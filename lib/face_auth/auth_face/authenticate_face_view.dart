...
import 'package:vote_secure/controllers/auth_controller.dart';
...
import '../../widgets/scanning_animation/animated_view.dart';
import '../register_face/camera_view.dart';
...
class _AuthenticateFaceViewState extends State<AuthenticateFaceView> {
  ...
  get _playScanningAudio => _audioPlayer
    ...
    ..play(AssetSource("audio/scan_beep.wav"));

  get _playFailedAudio => _audioPlayer
    ..stop()
    ..setReleaseMode(ReleaseMode.release)
    ..play(AssetSource("audio/failed.mp3"));
...
// A function to calculate the Euclidean distance between two points
  ...
  _fetchUsersAndMatchFace() {
    Get.find<AuthController>().loginUser(widget.email, widget.password);
    String uid = Get.find<UserController>().currentUser!.uid;
  ...
  _matchFaces(UserModel user) async {
